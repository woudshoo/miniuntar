...
(defun unpack-tar-stream (stream &key (directory *default-pathname-defaults*))
  (let ((block (make-block-buffer)))
    (loop
...
(defun unpack-tar-sequence (sequence &key (directory *default-pathname-defaults*))
  (flexi-streams:with-input-from-sequence (stream sequence)
    (unpack-tar-stream stream :directory directory)))
...
(defun contents-tar-stream (stream)
  (let ((block (make-block-buffer))
        (result '()))
    (loop
       (let ((size (read-sequence block stream)))
...
(defun contents-tar-sequence (sequence)
  (flexi-streams:with-input-from-sequence (stream sequence)
    (contents-tar-stream stream)))
